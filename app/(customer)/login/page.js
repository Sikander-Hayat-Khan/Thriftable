...
function LoginContent() {
  ...
  return (
    <div className="min-h-screen pt-16 sm:pt-20 pb-20 px-6 sm:px-10 lg:px-16 max-w-7xl mx-auto">
      ...
      <div className="mb-8 mt-2 flex flex-wrap items-center justify-between gap-4 border-b border-black/10 dark:border-white/10 pb-5">
        <div className="flex items-center gap-2 text-xs font-mono tracking-widest uppercase text-neutral-500">
          <Link href="/" className="hover:text-neutral-900 dark:hover:text-white transition-colors">
            Home
          </Link>
          <span>/</span>
          <span className="text-[#B2A376] font-semibold">Sign In</span>
        </div>
      </div>

      {/* Main Split Layout: Left Image (50%) | Right Sign In Form (50%) */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-stretch">
        {/* Left Column: High Quality Image (50%) */}
        <div className="w-full relative flex">
          <div className="relative w-full h-95 sm:h-120 lg:h-auto min-h-115 overflow-hidden border border-black/15 dark:border-white/15 bg-neutral-900 shadow-xl group">
            <Image
        ...
        {/* Right Column: Sign In Form (50%) */}
        <div className="w-full flex flex-col justify-center">
          <div className="p-6 sm:p-10 bg-white dark:bg-neutral-900/90 flex flex-col gap-6">
            {/* Header */}
            <div>
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-logo font-bold uppercase tracking-wider text-neutral-900 dark:text-white">
                {mode === "forgot" ? "Reset Password" : "Member Sign In"}
              </h1>
              <p className="text-xs font-proda text-neutral-600 dark:text-neutral-300 mt-1">
                {mode === "forgot"
                  ...
                  : "Welcome back. Enter your credentials to access your archive."}
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  className="w-full px-4 py-3.5 rounded-none bg-neutral-50 dark:bg-neutral-950 border border-black/15 dark:border-white/15 text-neutral-900 dark:text-white placeholder-neutral-400 text-sm focus:outline-none focus:border-[#B2A376] transition-colors font-mono"
                />
              </div>

              {/* Password (if signin mode) */}
              {mode === "signin" && (
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="text-[11px] font-mono text-[#807248] dark:text-[#d3c59a] hover:underline underline-offset-4 cursor-pointer uppercase tracking-wider"
                    >
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="••••••••"
                      className="w-full px-4 py-3.5 rounded-none bg-neutral-50 dark:bg-neutral-950 border border-black/15 dark:border-white/15 text-neutral-900 dark:text-white placeholder-neutral-400 text-sm focus:outline-none focus:border-[#B2A376] transition-colors font-mono pr-12"
                    />
                    <button
                      type="button"
                      onClick={() => setShowPassword(!showPassword)}
                      className="absolute right-3.5 top-1/2 -translate-y-1/2 text-neutral-400 hover:text-neutral-900 dark:hover:text-white p-1 cursor-pointer"
              ...
              {/* Submit Button with Swipe Animation */}
              <button
                type="submit"
                ...
                <span className="relative z-10 text-black group-hover:text-white dark:group-hover:text-black transition-colors duration-300 flex items-center justify-center gap-2">
                  {isSubmitting ? (
                    <>
                      ...
                      <span>{mode === "signin" ? "Authenticating..." : "Sending Reset Link..."}</span>
                    </>
                  ) : mode === "signin" ? (
            ...
            <div className="pt-4 border-t border-black/10 dark:border-white/10 text-center text-xs font-mono">
              {mode === "forgot" ? (
                <button
                  ...
                  className="text-[#807248] dark:text-[#d3c59a] font-bold uppercase tracking-wider hover:underline underline-offset-4 cursor-pointer"
                >
                  ← Return to Sign In
                </button>
              ) : (
                <>
                  <span className="text-neutral-500">Not a member yet? </span>
                  <Link
                    href={`/signup${next !== "/dashboard" && next !== "/" ? `?next=${encodeURIComponent(next)}` : ""}`}
