...
export function WishlistToastItem({ item, onViewWishlist, closeToast }) {
  if (!item) return null;
...
export function showWishlistToast(item, router) {
  if (!item) return;
  ...
  toast(
    ...
    {
      toastId: `wishlist-toast-${item.id}-${Date.now()}`,
